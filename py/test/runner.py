...
class Pokemon3dTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            Pokemon3dTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = Pokemon3dTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = Pokemon3dTestRunner.getenv("POKEMON3D_TEST_LIVE")
        override = Pokemon3dTestRunner.getenv("POKEMON3D_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = Pokemon3dTestRunner.getenv(key)
        ...
        explain = Pokemon3dTestRunner.getenv("POKEMON3D_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["POKEMON3D_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if Pokemon3dTestRunner._test_control is not None:
            return Pokemon3dTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                Pokemon3dTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            Pokemon3dTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = Pokemon3dTestRunner.load_test_control()
...
def load_env_local():
    Pokemon3dTestRunner.load_env_local()


def env_override(m):
    return Pokemon3dTestRunner.env_override(m)


def entity_list_to_data(lst):
    return Pokemon3dTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return Pokemon3dTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return Pokemon3dTestRunner.load_test_control()


def live_delay_ms():
    return Pokemon3dTestRunner.live_delay_ms()
