...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("POKEMON3D_TEST_LIVE") == "TRUE" ||
		os.Getenv("POKEMON3D_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("POKEMON3D_TEST_EXPLAIN"); explain != "" {
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.Pokemon3dSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.Pokemon3dError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.Pokemon3dSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.Pokemon3dError{Msg: msg, Code: code}
}
