...
The Pokemon3D API provides access to 3D models of Pokémon, enabling developers to easily integrate and display interactive Pokémon models within their applications.

Learn more about Pokemon3D API at [pokemon3d.io](https://pokemon3d.io).
...
This SDK exposes the API as a small set of **semantic entities** — Pokemon — that you
...
const items = await client.Pokemon().list()
```
...
```go
client := sdk.Test()
result, err := client.Pokemon(nil).List(
    nil, nil,
)
```
