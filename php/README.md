...
The PHP SDK for the Pokemon3d API — an entity-oriented client using PHP conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `$client->Pokemon()` — with named operations (`list`/`load`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
```php
require_once 'pokemon3d_sdk.php';
$client = new Pokemon3dSDK($options);
```
...
```
php/
├── pokemon3d_sdk.php          -- Main SDK class
...
The main class (`pokemon3d_sdk.php`) exports the SDK class
