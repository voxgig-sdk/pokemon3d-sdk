...
class PokemonEntity
{
    ...
    {
        if ($args) {
            $this->_data = Pokemon3dHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = Pokemon3dHelpers::to_map(Struct::clone($args)) ?? [];
     ...
     *   SDK boundary; throws Pokemon3dError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = Pokemon3dHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
     ...
     *   the SDK boundary; throws Pokemon3dError on failure (item-5 convention).
     */
