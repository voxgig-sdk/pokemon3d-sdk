...
// Pokemon3d SDK utility: make_point

require_once __DIR__ . '/../core/Helpers.php';

class Pokemon3dMakePoint
{
    public static function call(Pokemon3dContext $ctx): array
    {
        if (isset($ctx->out['point'])) {
            ...
            if ($ctx->out['point'] instanceof Pokemon3dError) {
        ...
        if (count($op->points) === 1) {
            ...
            foreach ($op->points as $p) {
                $point = $p;
                $select_def = Pokemon3dHelpers::to_map(\Voxgig\Struct\Struct::getprop($p, 'select'));
                $found = true;
            ...
            if ($reqselector) {
                $req_action = \Voxgig\Struct\Struct::getprop($reqselector, '$action');
                if ($req_action && $point) {
                    $point_select = Pokemon3dHelpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'select'));
