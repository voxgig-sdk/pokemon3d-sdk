...
//     (\`new Pokemon3dSDK(...)\` / \`Pokemon3dSDK.test(...)\` become
//     \`Pokemon3dSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"pokemon":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new Pokemon3dSDK(...)\` and any
// \`Pokemon3dSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+Pokemon3dSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/Pokemon3dSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
