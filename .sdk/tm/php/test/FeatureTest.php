...
class FtCtrl extends Pokemon3dControl
{
...
class FtClient
{
    ...
    private Pokemon3dUtility $utility;

    public function __construct(Pokemon3dUtility $utility, array $options)
    {
    ...
    public function get_utility(): Pokemon3dUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (Pokemon3dContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public Pokemon3dContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (Pokemon3dContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = Pokemon3dConfig::make_feature($name);
    ...
    private static function buildUrl(Pokemon3dSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof Pokemon3dError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new Pokemon3dResponse($fetched);
        }
    ...
    private function fail(Pokemon3dContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (Pokemon3dContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
