...
// Pokemon3d SDK primary utility test
...
class PrimaryUtilityTest extends TestCase
{
    ...
    private static function make_ctx_from_map(
        ?array $ctxmap,
        Pokemon3dSDK $client,
        Pokemon3dUtility $utility
    ): Pokemon3dContext {
        if ($ctxmap === null) {
            $ctxmap = [];
        }

        $ctx = new Pokemon3dContext($ctxmap, null);
    ...
    private static function fixctx(Pokemon3dContext $ctx, Pokemon3dSDK $client): void
    {
    ...
    private static function err_from_map(?array $m): ?Pokemon3dError
    {
        if ($m === null) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new Pokemon3dError($code, $msg);
    }
    ...
    public function test_make_error_no_throw(): void
    {
        ...
        $ctx->result = new Pokemon3dResult([
            'ok' => false,
            'resdata' => ['id' => 'safe01'],
        ]);
    ...
    public function test_feature_add_basic(): void
    {
        ...
        $feature = new Pokemon3dBaseFeature();
    ...
    {
        $hook_client = Pokemon3dSDK::test(null, null);
        $hook_utility = $hook_client->get_utility();
        $ctx = self::make_test_ctx($hook_client, $hook_utility);

        $called = false;
        $hook_feature = new class extends Pokemon3dBaseFeature {
            public $hook_fn = null;
            public function TestHook(Pokemon3dContext $ctx): void
            {
    ...
    {
        $init_client = Pokemon3dSDK::test(null, null);
        ...
        $init_called = false;
        $feature = new class extends Pokemon3dBaseFeature {
            public $init_fn = null;
    ...
    {
        $blocked_client = new Pokemon3dSDK([
            'system' => [
    ...
    public function test_make_result_no_result(): void
    {
        ...
        $utility = $client->get_utility();
        $ctx = self::make_test_full_ctx($client, $utility);
        $ctx->spec = new Pokemon3dSpec(['step' => 'start']);
        $ctx->result = null;
    ...
    public function test_make_spec_basic(): void
    {
        ...
        $spec_client = Pokemon3dSDK::test(null, $setup_opts);
    ...
    public function test_prepare_auth_basic(): void
    {
        ...
        $auth_client = Pokemon3dSDK::test(null, $setup_opts);
    ...
    public function test_result_basic_basic(): void
    {
        ...
        $this->runset(self::get_spec($primary, 'resultBasic', 'basic'), function (array $entry) use ($client, $utility) {
            ...
            if ($result->err !== null) {
                $err_msg = ($result->err instanceof Pokemon3dError) ? $result->err->msg : (string)$result->err;
