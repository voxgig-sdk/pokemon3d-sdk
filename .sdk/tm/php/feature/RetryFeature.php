...
class Pokemon3dRetryFeature extends Pokemon3dBaseFeature
{
    ...
    public function init(Pokemon3dContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (Pokemon3dContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(Pokemon3dContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(Pokemon3dContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
