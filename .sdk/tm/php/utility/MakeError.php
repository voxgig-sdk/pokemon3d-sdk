...
class Pokemon3dMakeError
{
    public static function call(?Pokemon3dContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new Pokemon3dContext([], null);
        }
        $op = $ctx->op ?? new Pokemon3dOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new Pokemon3dResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof Pokemon3dError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new Pokemon3dError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof Pokemon3dError) {
