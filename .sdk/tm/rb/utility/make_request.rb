...
module Pokemon3dUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = Pokemon3dResponse.new({})
    ...
    if fetch_err
      ...
      response = Pokemon3dResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = Pokemon3dResponse.new(fetched)
    else
