// Command Pokemon3d-cli is a CLI + REPL for the Pokemon3d SDK,
...
const prompt = "pokemon3d"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "pokemon"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      POKEMON3D_APIKEY (api key), POKEMON3D_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: POKEMON3D_APIKEY carries the API key and
	// POKEMON3D_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("POKEMON3D_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("POKEMON3D_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewPokemon3dSDK(opts)
