...
AQL-driven command-line client **and** interactive REPL for the Pokemon3d
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/pokemon3d-cli)
make build
...
./pokemon3d-cli list pokemon
./pokemon3d-cli load 1 pokemon            # {id:1} shorthand
./pokemon3d-cli load '{id:1}' pokemon       # explicit match map

# 5. Override the API base URL for a single call
POKEMON3D_BASE=https://api.example.com ./pokemon3d-cli list pokemon

# 6. No arguments -> interactive REPL
./pokemon3d-cli
pokemon3d> list pokemon
pokemon3d> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/pokemon3d-cli
   ```
...
3. **Run a query.** Evaluate an AQL expression against the API (or run with no
   ...
   ```sh
   ./dist/*/pokemon3d-cli list pokemon
   ```
...
```sh
./pokemon3d-cli load 1 pokemon          # scalar shorthand for {id:1}
./pokemon3d-cli load '{id:1}' pokemon     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `pokemon3d>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an AQL atom (e.g. `pokemon`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/pokemon3d-cli`. |
...
(`./pokemon3d-cli <expr>`) and interactively (the REPL), and expressions compose the
