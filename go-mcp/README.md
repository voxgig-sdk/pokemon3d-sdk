...
MCP server exposing the Pokemon3d SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `pokemon3d_list` | `entity`, optional `query` map | First page of records as JSON |
| `pokemon3d_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `pokemon3d_list` and
`pokemon3d_load` tools become available in new sessions.
