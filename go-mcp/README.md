...
[MCP](https://modelcontextprotocol.io) server exposing the Pokemon3d SDK as
two agent tools — `pokemon3d_list` and `pokemon3d_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/pokemon3d-mcp)
make build

# 2. Provide credentials via the environment
export POKEMON3D_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user pokemon3d \
  -- /absolute/path/to/pokemon3d-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./pokemon3d-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// pokemon3d_list: first page of records
{ "entity": "pokemon" }
{ "entity": "pokemon", "query": { } }

// pokemon3d_load: one record by id
{ "entity": "pokemon", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/pokemon3d-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user pokemon3d \
     -- "$PWD"/dist/*/pokemon3d-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `pokemon3d_list` and `pokemon3d_load` tools now appear
   in new sessions. Ask the agent to *"list pokemon using pokemon3d"*
   and it calls `pokemon3d_list` with `{"entity":"pokemon"}`.
...
```sh
export POKEMON3D_APIKEY=sk_live_xxx            # API key
export POKEMON3D_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `pokemon3d_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `pokemon3d_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `POKEMON3D_APIKEY` | API key sent with every request. |
| `POKEMON3D_BASE` | Optional override of the API base URL. |
...
```sh
./pokemon3d-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"pokemon3d_load","arguments":{"entity":"pokemon","query":{"id":1}}}}'
```
