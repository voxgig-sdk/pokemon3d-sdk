...
class PokemonEntity
  ...
  def data_set(args)
    if args
      @_data = Pokemon3dHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = Pokemon3dHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Pokemon, Hash] the loaded Pokemon; raises Pokemon3dError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = Pokemon3dHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Pokemon>, Array] the matching Pokemon items; raises Pokemon3dError on failure
